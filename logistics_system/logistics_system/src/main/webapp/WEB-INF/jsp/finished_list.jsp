...
<font face="Microsoft YaHei UI light" color="#FFFFFF">已完成列表</font>
</div>
...
<c:forEach items="${tList}" var="finished">
  			<tr>
            <td style="text-align:center; width:15%; height:50px">
            ${finished.waybill_id}
            </td>
            <td style="width:45%;height:50px"><font face="Microsoft YaHei UI light" color="#000000"><a href="ShowFinished/showone?waybill_id=${ finished.waybill_id}">${finished.trade_id}</a></font>
            </td>
            <td style="width:10%;height:50px"><font face="Microsoft YaHei UI light" color="#000000">${finished.good_source}</font>
            </td>
            <td style="width:10%;height:50px"><font face="Microsoft YaHei UI light" color="#000000">${finished.good_destination}</font>
            </td>
            <td style="width:10%;height:50px"><font face="Microsoft YaHei UI light" color="#000000">${finished.good_trs_time}</font>
            </td>
            <td style="width:10%;height:50px"><font face="Microsoft YaHei UI light" color="#000000"><button  class="btn btn-default btn-primary" onclick="arrive(${ finished.waybill_id})">查看</button></font>
            </td>
            </tr>
            </c:forEach>
</table>
	...
	<div class="row clearfix">
	 ...
	 <script>
 	   ...
 	   location.href="ShowFinished/page?pageNum="+pageNum+"&maxPage="+${pageInfo.pages};
 	   }
 	    function next(){
 	   location.href="ShowFinished/page?pageNum="+${pageInfo.pageNum+1}+"&maxPage="+${pageInfo.pages};
 	   }
 	   function back(){
 	   location.href="ShowFinished/page?pageNum="+${pageInfo.pageNum-1}+"&maxPage="+${pageInfo.pages};
 	   }
    ...
    {
        window.location.href='ShowFinished/showone?waybill_id='+waybill_id;
    }
 	   </script>
  </body>
</html>
