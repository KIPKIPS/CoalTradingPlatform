...
<!-- table代码 -->
<table class="table table-hover table-bordered" style="max-height: 300px;width: 900px; margin:0 auto;">
...
<div style="width:300px; margin:0 auto;">
    <ul class="pagination" id="pageList" style="line-height: 100px">
        ...
        <li style="display: none" id="spanpage1" class="active"><a href="javascript:nPage(1)" id="qwe">1</a></li>
        <li id="spanNext" ><a href="javascript:next()">&raquo;</a></li>
    </ul>
    	<div style="width:500px;margin-top: -55px;margin-left: 80px">	<span class = 'span1' style="margin-right: 10px">当前第<span id="pageNo"></span>页/总共<span id="pageSum"></span>页 </span>
    	跳转到<input onkeyup="value=value.replace(/[^\d\.]/g,'')" id="nextPage" style="width: 30px"><input  onclick="toPagen(document.getElementById('nextPage').value)" style="margin-left: 10px" class = 'btn btn-default btn-primary' type='button' id='pageskip' value='跳转' >
    	</div>
...
<script>
    ...
    var pageSum = 0;
 	...
 	function getInfo2(){
		...
		$.ajax({
		type:"post",
   		url:"http://10.61.88.30:8082/logistics_system/waybill/getState1InfoByList",
   		dataType: "json", 
		...
		if(data!=null)
   			{
   				...
   				$("#tablePage").append("<tr><th>"+data[i].trade_id+"</th><th>"+data[i].waybill_id+"</th><th>"+data[i].distributor+"</th><th>"+data[i].dis_phone+"</th><<th>"+data[i].good_source+"</th><th>"+data[i].good_destination+"</th></tr>");
