...
<html>
  <head>
    ...
    <title>My JSP 'verifyNewsInfoPage.jsp' starting page</title>
	...
	<script>
		...
		$.ajax({
			...
			success : function(msg) {			    
				...
				$("#newsList").append("<div id='newsListHeader'><span id='headerSpanA'>资讯编号</span><span id='headerSpanB'>资讯编辑员编号</span><span id='headerSpanC'>资讯标题</span><span id='headerSpanD'>资讯发布时间</span></div>");
				jQuery.each(JSON.parse(newsList), function(i,item){
					...
					$("#newsList").append("<div id = 'divnewsList" + i + "' class = 'newsListColor" + rand + "'><a href='News/getNewsById?nID="+item.nID+"'><p><span class='span1'>" + item.nID + "</span><span class='span2'>" + item.nEditorID + "</span><span class='span3'>" + item.nTheme + "</span><span class='span4'>" + item.nTime + "</span></p></a></div>")
				...
				$('#changePage').append("<center id='center1' style='font-size:13px;'></center>")
				...
				$('#center1').append("<input class = 'inputpage' id = 'input1' value='"+JSON.parse(newsPageInfo).pageNum+"' type='text'>&nbsp;&nbsp;");
    			$('#input1').bind("input propertychange",function() {
 					$("#input1").val($("#input1").val().replace(/[^\d]/g,'')); 
				});
			 	$('#center1').append("<input type='button' class = 'btn btn-default btn-primary' id='pageskip' value='跳转' >");
