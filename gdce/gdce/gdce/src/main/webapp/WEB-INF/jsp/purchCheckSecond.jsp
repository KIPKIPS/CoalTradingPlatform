...
<html>
  <head>
    ...
    <title>My JSP 'purch999.jsp' starting page</title>
  ...
  <body>
	...
	<table id="tblMain" class="table table-hover table-bordered">
	 ...
	 <tr><th colspan=11>待审核</th></tr>
    		...
    		<tbody>
    	<c:forEach var="listing" items="${purchCheckSecondList}">
    		<tr>
    			...
    			<th>   				
    				<button class="btn btn-default btn-primary" onclick="rem(${listing.pNumber},${listing.uID})">查看</button>
    			</th>    			
    		</tr>
    		</c:forEach>
    		</tbody>
 	 ...
 	 <script>
 	   	...
 	   	var pageNum=document.getElementById("pageNum").value
 	   		if(pageNum)
 	   		location.href="SellerGoodInfo/page?pageNum="+pageNum+"&maxPage="+${pageInfo.pages}+"&flag=6";
 	   	}
 	   function next(){
 	   location.href="SellerGoodInfo/page?pageNum="+${pageInfo.pageNum+1}+"&maxPage="+${pageInfo.pages}+"&flag="+6;
 	   }
 	   function back(){
 	   location.href="SellerGoodInfo/page?pageNum="+${pageInfo.pageNum-1}+"&maxPage="+${pageInfo.pages}+"&flag="+6;
 	   }
		function rem(pNumber,uID){
      		...
      		location.href="SellerGoodInfo/purchCheckSpecificInfo?pNumber="+pNumber+"&uID="+uID;	
   	  	}
   	  	/*     表格隔行变色 */
